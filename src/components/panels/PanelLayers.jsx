...
export const PanelLayers = () => {
  const { setConfigMap, configMap, setTool} = useContext(storeContext)
    ...
    return (
        <div className='panel_layers bg-light shadow' onMouseLeave={() => setTool('')}>
            ...
            <div className='overflow-scroll' style={{height: '100%'}}>
                {
                    layersTile.map(x => (
                        <div className={`card p-2 ${configMap.layer == x.url && 'bg-warning'}`} style={{ width: '200px' }} onClick={() => setConfigMap({ ...configMap, layer: x.url })}>
